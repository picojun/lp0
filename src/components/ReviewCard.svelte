<script lang="ts">
    export let reviewItem: ProductReview;
    export let left: boolean;
</script>

<div class={"flex flex-col gap-2 w-full max-w-[500px] mx-auto relative " + 
    (left ? reviewItem.index % 2 === 1 ?
    'hidden md:flex opacity-0 pointer-events-none max-h-[40px] overflow-hidden' : ' ' :
    reviewItem.index % 2 === 0 ? 
    'hidden md:flex opacity-0 pointer-events-none max-h-[40px] overflow-hidden' : ' ')}>
    <div class="flex relative">
        <div class={'absolute rounded-full bg-white aspect-square hidden md:grid place-items-center h-4 top-0 ' + (reviewItem.index % 2 === 0 ? ' right-0 -mr-8 translate-x-1/2' :
